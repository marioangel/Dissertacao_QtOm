...
A Figura~\ref{fig:conformance} mostra o resultado da comparação descrita anteriormente, assim como, em testes anteriores os conjuntos de testes estão divididos em \textit{containers} associativos compostos por QHash, QSet, QMap, QMultiHash e QMultiMap e \textit{containers} sequenciais compostos por QList, QQueue, QLinkedList, QStack e QVector. Vale ressaltar que QtCreator por ser o ambiente de desenvolvimento padrão para criação de aplicações que usam o \textit{framework} Qt, obteve um nível de conformidade de 100\% em todos os conjuntos de testes que compõe \textit{esbmc--qt} em todas as condições possíveis de acordo com \textit{The Qt Documentation}~\cite{qtdocumentation}. Entretanto, ao se analisar o comportamento do compilador Clang junto com o modelo operacional proposto (QtOM), se obtem um nível de conformidade com média de 94,320\% em todos os conjuntos de testes que compõe \textit{esbmc--qt} em todas as condições possíveis de acordo com \textit{The Qt Documentation}~\cite{qtdocumentation}. 

Ao se analisar de modo particular, os conjuntos de testes QMultiHash, QMultiMap, QQueue, QStack obtiveram um nível de 100\% de semalhança comparado aos resultados obtidos por QtCreator. O conjunto de teste QList obteve o menor nível de semelhança em relação a QtCreator com 84,677\%, isso ocorreu devido algumas funções presente na classe \textit{QList} não possuirem uma representação dentro do modelo operacional proposto, o mesmo problema também ocorreu com o conjunto de teste QVector que no caso obteve um nível de semelhança de 90,789\%, este conjunto é considerado o maior em números de casos de teste. Vale ressaltar que o modelo operacional desenvolvido é usado junto com ferramentas de verificação que possuem limitações em relação as linguagens bases utilizadas o que consequentemente afeta na criação das representações de algumas funções, não sendo possível a criação de certas representações (\textit{i.e.} fromSet, fromStdList, fromVector, etc). Os conjuntos QLinkedList e QMap, respectivamente, obtiveram nivéis de semelhança iguais a 89,655\% e 91,919\% mas foram os conjuntos de teste que mais apresentaram falsos positivos e falsos negativos devido a problemas em suas representações com o uso de iterators de tipos constantes. Por fim, o conjunto QHash teve o maior nível de semelhança obtido entre os demais conjuntos em relação a QtCreator com 94,667\% mas problemas com representações de funções que expressavam sobrecarga de metodos criaram alguns falsos positivos e falsos negativos e o conjunto QSet obteve um nível de semelhança de 91,489\%, devido a problemas nas representações que expressam os operadores (adição, união, etc.) e na falta de algumas representações no modelo operacional também. 
...
Também foram descrito os resultados obtidos a cerca da verificação de todos os casos de teste presentes na suíte de teste \textit{esbmc--qt} utilizando ferramentas de verificação distintas (ESBMC$++$, LLBMC e DIVINE) com o objetivo de analisar a corretude e a eficácia do modelo operacional proposto junto dessas ferramentas. Cada processo de verificação e as limitações das ferramentas são descritos. Vale ressaltar que o ESBMC$++$ foi capaz de identificar cerca de $89,3$\% dos erros nos casos de teste utilizados (\textit{i.e.}, $631$ dos $708$ casos de testes utilizados possuem erros), LLBMC e DIVINE apresentam, respectivamente, taxas com $81,4$\% e $89$\% (\textit{i.e.}, $576$ e $630$ dos $708$ casos de teste utilizados possuem erros). Como consequência, a metodologia proposta não apenas se limita a uma determinada ferramenta, podendo-se adaptar para aplicações específicas em que algumas abordagens sejam mais adequedas do que outras. 

Por fim, foi descrito os resultados obtidos atráves da verificação de 2 aplicações reais \textit{Locomaps} e \textit{GeoMessage Simulator} utilizando QtOM. O ESBMC$++$ junto ao modelo operacional proposto (QtOM) foi aplicado para verificar as aplicações \textit{Locomaps} e \textit{GeoMessage Simulator} buscando verificar as seguintes propriedades: violação dos limites de um array, estouros aritméticos, divisão por zero, segurança de ponteiro e outras propriedades específicas do \textit{framework} definidas em QtOM. O processo de verificação de ambas as aplicações com a metodologia proposta levou aproximadamente $6,7$ segundos para gerar $32$ VCs para \textit{Locomaps} e $16$ segundos para gerar $6421$ VCs para \textit{GeoMessage Simulator} em um comum \textit{desktop} onde ambas aplicações possuiam o mesmo erro.
