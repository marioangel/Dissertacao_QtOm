...
Neste capítulo, foi descrito o subjconjunto denominado \textit{Qt Container} que se faz presente no módulo \textit{Qt Core} do \textit{framework} multiplataforma Qt, sendo em classificado em dois subgrupos: sequenciais e associativos, dependendo da estrutura de armazenamento implementada, logo em seguida, é descrito a linguaguem base formalizada por Ramalho \textit{et al.}~\cite{ECBS13} mas adaptada com o objetivo de formular adequadamente a verificação de ambos tipos de \textit{containers} existentes. Por fim, os \textit{containers} sequenciais e associativos foram descritos de forma detalhada, a fim de ressaltar todos os metódos que constituem essas bibliotecas, assim como, a forma que estão implementados a partir da transformação SSA realizada.
